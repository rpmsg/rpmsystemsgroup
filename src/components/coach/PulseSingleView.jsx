...
import { ROLE_COLORS, ADMIN_LABELS_FULL } from '../../constants'
import { buildGroups, buildRoomingPairs } from '../../lib/pulseAlgorithms'
...
export default function PulseSingleView({ team, defaultScores, scoresByAdmin, availableAdmins, roster }) {
  ...
  const [numGroups, setNumGroups] = useState(
    () => Math.min(6, Math.max(3, Math.ceil((defaultScores?.length || 0) / 5)))
  )
  ...
  return (
    <>
      ...
      {availableAdmins?.length > 0 && (
        <div className="no-print" style={{ padding: '10px 24px', borderBottom: '1px solid var(--bdr)', background: 'var(--d2)', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          >
            {availableAdmins.map(n => (
              <option key={n} value={n}>{ADMIN_LABELS_FULL[n]}</option>
            ))}
          </select>
        </div>
      )}
