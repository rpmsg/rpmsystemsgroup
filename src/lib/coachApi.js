...
export async function fetchDashboardData(teamId) {
  const [teamRes, rosterRes, scoresRes, panicRes, nominationsRes] = await Promise.all([
    supabase.from('teams').select('id, name, team_code, season').eq('id', teamId).single(),
    supabase.from('roster').select('id, full_name, status').eq('team_id', teamId).order('full_name'),
    supabase.from('pulse_report_scores').select('athlete_name, positive_mentions, negative_mentions, social_role').eq('team_id', teamId).order('positive_mentions', { ascending: false }),
